...
const PUBLIC_ROUTES = ['/', '/login', '/register', '/contact', '/about', '/pricing']
const AUTH_ROUTES = ['/login', '/register']
const PROTECTED_ROUTES = ['/dashboard', '/projects', '/requirements', '/collections', '/settings']
...
export async function middleware(request: NextRequest) {
  ...
  if (!firebaseToken && PROTECTED_ROUTES.some(route => pathname.startsWith(route))) {
  ...
  if (firebaseToken && AUTH_ROUTES.some(route => pathname.startsWith(route))) {
