...
export default function RequirementItem({ requirement }: RequirementItemProps) {
  return (
    <div className="space-y-6 p-4 border rounded-lg">
      ...
      <p className="text-muted-foreground line-clamp-2">{requirement.description}</p>
      ...
      {requirement.tags && requirement.tags.length > 0 && (
        <div className="flex flex-wrap gap-2">
          {requirement.tags.slice(0, 3).map((tag: string, index: number) => (
            <Badge key={index} variant="secondary">{tag}</Badge>
          ))}
          {requirement.tags.length > 3 && (
            <Badge variant="secondary">+{requirement.tags.length - 3}</Badge>
          )}
        </div>
      )}
    </div>
