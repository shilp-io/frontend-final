...
export function AsciiTable<T>({
  ...
  const createBorder = (
    ...
    return (
      start + columns.map((col) => mid.repeat(col.width + 2)).join(sep) + end
    );
  };
  ...
  const createRow = (cells: string[]) => {
    return (
      "│ " +
      cells.map((cell, i) => padContent(cell, columns[i].width)).join(" │ ") +
      " │"
    );
  };
    ...
    <div className={ASCII_STYLES.container}>
      ...
      {data.map((item, index) => (
        <div
          key={index}
          className={
            onRowClick || handleGoToPage ? ASCII_STYLES.row : undefined
          }
          onClick={() => {
            if (onRowClick) {
              onRowClick(item);
