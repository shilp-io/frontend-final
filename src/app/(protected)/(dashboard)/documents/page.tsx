	...
	const renderDetails = (doc: ExternalDoc) => (
		<div className="space-y-6">
			...
			<div className="space-y-2">
				...
				<p className="text-muted-foreground">Created: {formatDate(doc.created_at)}</p>
				<p className="text-muted-foreground">Last Modified: {formatDate(doc.updated_at)}</p>
				<p className="text-muted-foreground">URL: <a href={doc.url} target="_blank" rel="noopener noreferrer" className="text-primary hover:underline">{doc.url}</a></p>
				{doc.author && <p className="text-muted-foreground">Author: {doc.author}</p>}
				{doc.version_info && <p className="text-muted-foreground">Version: {doc.version_info}</p>}
				{doc.publication_date && <p className="text-muted-foreground">Published: {formatDate(doc.publication_date)}</p>}
				{doc.last_verified_date && <p className="text-muted-foreground">Last Verified: {formatDate(doc.last_verified_date)}</p>}
